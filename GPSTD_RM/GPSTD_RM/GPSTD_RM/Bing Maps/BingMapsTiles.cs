...
namespace GPSTD_RM
{
    ...
    public class BingMapsTiles
    {
        ...
        private void GetActivePlaneImages(GeoCoordinate centerCoordinate)
        {
            ...
            centerPixelXY.X = (int)(centerPixelXY.X / tileDimensions.X) * tileDimensions.X;
            centerPixelXY.Y = (int)(centerPixelXY.Y / tileDimensions.Y) * tileDimensions.Y;
            ...
            for (int xIndex = 0; xIndex < ActiveTilePlaneSize; xIndex++)
            {
                ...
                for (int yIndex = 0; yIndex < ActiveTilePlaneSize; yIndex++)
                {
                    ...
                    cellInformation.name = "" + zoomLevel + "_" + tileCenterPixelXY.X + "_" + tileCenterPixelXY.Y;
                    string fileName = cellInformation.name + ".png";
                        ...
                        using (var store = IsolatedStorageFile.GetUserStoreForApplication())
                            if (store.FileExists(fileName))
                            {
                                ...
                                //using (var reader = new StreamReader(readStream))
                                {
                                    try
                                    {
                                        cellInformation.Image = Texture2D.FromStream(SpriteBatch.GraphicsDevice, readStream);
                                        imageread = true;
                                    }
        ...
        private void SetTileImage(OpenReadCompletedEventArgs e, TileInformation tileInformation)
        {
            ...
            string fileName = tileInformation.name + ".png";
            using (var store = IsolatedStorageFile.GetUserStoreForApplication())
